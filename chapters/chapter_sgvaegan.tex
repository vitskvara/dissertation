...
\label{sec:theory}
Variational autoencoder (VAE) (see Chapter~\ref{sec:chapter_survey}) fits a generative model with data $\vc{x} \in \mathbb{R}^n$
\begin{align}
p(\vc{x}) & = \int p_{\vc{\theta}}(\vc{x} \vert \vc{z})p(\vc{z})dz & p_{\vc{\theta}}(x\vert \vc{z}) & =\mathcal{N}(\vc{x};g_{\vc{\theta}}(\vc{z}),\vc{\sigma}^{2} \mathbf{I}),\label{eq:vae}
\end{align}
where the prior on the latent $k$-dimensional variable $z$ is either fixed $p(\vc{z})=\mathcal{N}(0,\mathbf{I})$, or further parametrized $p_{\vc{\theta}}(\vc{z})$ as in~\cite{tomczak2017vae}. Distribution $p_{\vc{\theta}}(\vc{x}\vert \vc{z})$ is frequently called \emph{decoder} with $g_{\vc{\theta}}(\vc{z})$ being realized by a neural network parametrizing the mean of the normal distribution. To fit the model, VAE introduces an \emph{encoder}, which is a conditional probability distribution $q_{\vc{\phi}}(\vc{z}\vert x)$ parametrized similarly to the decoder as $q_{\vc{\phi}}(\vc{z}\vert \vc{x})=\mathcal{N}(\vc{z};\vc{\mu}_{\vc{\phi}}(\vc{x}),\text{diag}(\vc{\sigma}_{\vc{\phi}}(\vc{x})))$ with $\vc{\mu}_{\vc{\phi}}(\vc{x}), \vc{\sigma}_{\vc{\phi}}(\vc{x})$ being neural networks. During training, the weights $(\vc{\theta},\vc{\phi})$ of both neural networks are optimized to maximize the evidence lower bound (ELBO) on given training samples.

\subsection{Orthogonal generative model}
The above generative model~\eqref{eq:vae} can be rewritten as $\vc{x}=g_{\vc{\theta}}(\vc{z})+e$ where $e$ is (usually isotropic) Gaussian noise. This formulation  emphasizes the need for marginalization since $g_{\vc{\theta}}(\vc{z})$ is a random variable of dimension $k$ and noise $e$ is a random variable of dimension $n,$ which is the same as the data. Equation~\eqref{eq:vae} therefore marginalizes away random variables corresponding to latent $z.$ Due to this ambiguity, the assignment between $x$ and $z$ is not unique. In fact, for any $\vc{x}$ exists  $z'$ such that \[
\vc{x}=g_{\vc{\theta}}(\vc{z}')+e^{\bot}
\]
where $e^{\bot}$ is the observation noise that lies in the normal space perpendicular to the manifold defined by the decoder $g_{\vc{\theta}}(\vc{z})$~\cite{pidhorskyi2018generative, vsmidl2019anomaly}. This interpretation implies independence between $g_{\vc{\theta}}(\vc{z}')$ and $e^\bot$ which allows for the approximation $p(\vc{x})\approx p(g_{\vc{\theta}}(\vc{z}'))p(e^{\bot})$. 

However, the independence relation was found~\cite{vsmidl2019anomaly} to be sufficiently accurate even for the original noise estimate, $\vc{x}=\vc{x}'+e$, where $\vc{x}'=g_{\vc{\theta}}(\vc{z})$.  The  probability of the reconstruction $p(\vc{x}')$ is given by the change of coordinate formula from $p_{z}(\vc{z})$,  yielding
\begin{align}
p(\vc{x})\approx p(\vc{x}')p(e)=p_{z}(g_{\vc{\theta}}^{-1}(\vc{x}'))\left\vert \frac{\partial g_{\vc{\theta}}^{-1}(\vc{x})}{\partial \vc{x}}\right\vert p(e).\label{eq:pxjacodeco}
\end{align}
A gaussian noise model might be assumed, e.g. $p(e) = \mathcal{N}(\vc{x}-\vc{x}',\vc{\sigma}^{2}\mathbf{I})$. In practice, an anomaly score for the orthogonal model $s(\vc{x}) = - \log p(\vc{x})$ is computed as
\begin{align}
s(\vc{x}) & = - \log p(e) -\log p_{z}(\vc{z})-\log\left\vert \frac{\partial g_{\vc{\theta}}^{-1}(\vc{x})}{\partial \vc{x}}\right\vert ,\label{eq:jacodeco}
\end{align}
which is essentially the reconstruction error with additional terms.

\subsection{Anomaly in the latent space}
Let's now observe how the above generative model changes when we assume the distribution on latent $z$ to be multi-modal conditioned by hidden label $y$. Let
\begin{equation} \label{eq:pzy}
p(\vc{z}\vert y)=p_{n}(\vc{z})^{y}p_{a}(\vc{z})^{1-y},y\in[0,1],
\end{equation}
...
\begin{align*}
s(\vc{x} \vert y) & = - \log p(e) -y\log p_{n}(\vc{z})  -(1-y)\log p_{a}(\vc{z}) - \log\left\vert \frac{\partial g_{\vc{\theta}}^{-1}(\vc{x})}{\partial \vc{x}}\right\vert,
\end{align*}
Since $y$ is usually unknown, we will integrate it away by expectation over $y$, 
\begin{align}
\mathbb{E}_{p(y)} \left[  s(\vc{x} \vert y) \right]  = &  - \log p(e) - \log\left\vert \frac{\partial g_{\vc{\theta}}^{-1}(\vc{x})}{\partial \vc{x}}\right\vert \nonumber \\ 
 & - \int_{\mathcal{Y}} y\log p_{n}(\vc{z}) p(y) dy - \int_{\mathcal{Y}} (1-y)\log p_{a}(\vc{z}) p(y) dy \nonumber \\
   \propto & - \log p(e) - \log\left\vert \frac{\partial g_{\vc{\theta}}^{-1}(\vc{x})}{\partial \vc{x}}\right\vert  - \alpha\log p_{n}(\vc{z}) = s(\vc{x}). \label{eq:jacodeco2}
\end{align}
...
Now consider a \emph{disentagled} model, where the data is generated from multiple independent latent spaces, $z_{i}=z_{1},\ldots,z_{l}$, 
\begin{equation} \label{eq:multilatent}
\vc{x}=f(\vc{z}_{1},\ldots,z_{l})+e,
\end{equation}
where each of the latents has distribution $p(\vc{z}_{i})=\mathcal{N}(0,\mathbf{I})$. Using the same derivations, each of the latent spaces $z_i$ can be a potential source of different types of anomalies, with hidden labels  $y=(y_{1},\ldots,y_{l})   $. Therefore, instead of~\eqref{eq:pzy},  we have
\begin{equation} 
...
s(\vc{x}) & = - \log p(e) - \log\left\vert \frac{\partial g_{\vc{\theta}}^{-1}(\vc{x})}{\partial \vc{x}}\right\vert  -  \sum_{i=1}^{l}\alpha_{i}\log p_{n_{i}}(\vc{z}_{i}),
\end{align}
